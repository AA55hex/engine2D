...
class texture2D
{
 public:
   ...
   */
  texture2D(int width, int height, const void* data, const color_format format,
            const texture_filter filter = texture_filter::linear,
            const wrap_mode mode = wrap_mode::clamp_to_edge);
