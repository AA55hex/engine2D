...
{
  int x, y;
  const core::window_properties& prop =
      core::engine::get_window().get_properties();
  const float window_width{static_cast<float>(prop.width)};
  const float window_height{static_cast<float>(prop.height)};
...
const std::array<r_bind, 7> keyboard_relation{
...
static bool translate_key(SDL_KeyboardEvent event, keyboard_key& result)
{
  auto it = std::find_if(keyboard_relation.begin(), keyboard_relation.end(),
                         [&](const r_bind& buff) {
                           return buff.sdl_key == event.keysym.scancode;
                         });
...
const std::array<r_bind, 6> mouse_relation{
...
static bool translate_key(SDL_MouseButtonEvent event, mouse_button& result)
{
  auto it = std::find_if(
      ...
      [&](const r_bind& buff) { return buff.sdl_key == event.button; });
